# 머신러닝 프로세스 
# 데이터 정리 - 데이터 분리 (훈련/ 검증) - 알고리즘 - 모형학습 (훈련데이터) - 예측(검증데이터) - 모형 평가 - 모형 활용
# 회귀분석 : 독립변수(설명변수) -> 종속변수(예측변수)
# 단순 회귀분석 X -> Y 1 : 1  관계 (연속 변수 )
# Y = aX + b , 변수 X ,Y  -> a,b의 계수를 찾는 과정 
...
# 분석에 활용할 열(속성)을 선택 (연비 : Y , (실린더, 출력, 중량) : X )
ndf = df[['mpg', 'cylinders', 'horsepower', 'weight']]
print(ndf.head())   
print('\n')

### 종속 변수 Y인 "연비(mpg)"와 다른 변수 간의 선형관계를 그래프(산점도)로 확인
# Matplotlib으로 산점도 그리기 :plot(scatter)
...
sns.regplot(x='weight', y='mpg', data=ndf, ax=ax1)                 # 회귀선 표시 sns.regplot()
...
plt.show()
plt.close()
# x: weight , horse power -> y : mpg 선형관계 o 
...
r_square = lr.score(X_test, y_test)  # score(x,y) : 결정계수 구하는 함수 
print(r_square)   # R제곱 : 설명력 
print('\n')
