...
class Node : 
    ...
    def __init__(self, key: Any, value: Any, next: Node) -> None:
        ...
        self.key   = key    # 키
        self.value = value  # 값
        self.next  = next   # 뒤쪽 노드를 참조 (왜 이따구로 노드 정의했는지 이해 x , 3개로)
...
class ChainedHash:
    ...
    def __init__(self, capacity: int) -> None:
        """초기화"""
        self.capacity = capacity             # 해시 테이블의 크기를 지정 (해시 함수 기준이겠네)
...
# Do it! 실습 3-5[C]
    def search(self, key: Any) -> Any:
        ...
        hash = self.hash_value(key)  # 검색하는 키의 해시값을 구함 (해시함수)
        p = self.table[hash]         # 해시테이블의 인덱스(해시값)
